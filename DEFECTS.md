...
| P1 | D-18 | open | The documented `pnpm migrate:up` script is broken independently: `node --experimental-strip-types src/db/migrate.ts` cannot resolve its `./migrations.js` import, because type-stripping does not rewrite extensions. `node dist/db/migrate.js` works. The package.json script has never been runnable |
