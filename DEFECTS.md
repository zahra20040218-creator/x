...
| **P1** | **D-15** | **CLOSED** | Was: the fake snapshots the whole database and restores it on any failure, so concurrent rollbacks erased a committed write. **Verified against real PostgreSQL 17.11 on 2026-08-23**: one transaction commits while 20 concurrently roll back, and the committed row survives. The fake's divergence is real and is now bounded by an integration suite that runs on the real thing |
...
| **P0** | **M-1** | **CLOSED 2026-08-24** | Both apps now have complete Android host projects and BOTH BUILD. `√ Built app-debug.apk` — rider 158 MB (`iq.rideapp.rideapp_rider`), driver 159 MB (`iq.rideapp.rideapp_driver`). The driver's custom 124-line manifest survived intact. Fixed on the way: an undefined `${MAPS_API_KEY}` placeholder that failed the manifest merger, and a missing `@string/app_name` |
...
| **P1** | **M-3** | **CLOSED 2026-08-24** | `MapPickerView` in `packages/core` renders a real `GoogleMap` with a fixed centre pin. Eight distinct map states including permission-denied vs PERMANENTLY denied. 19 tests, all running with no device, no key and no network because the decision layer is a pure function. Without a key the map is not instantiated at all and offers no retry — a build-time key cannot be fixed by the user. **Rendering on a real device with a real key remains unverified** |
| **P1** | **A-1** | **FIXED** | Was: not an application - 4 files, no entry point, `vite build` failed with 0 modules. Now builds (329 modules), typechecks, lints, and **renders** - verified in a browser: login, RTL layout, and an explicit "Firebase config missing" state naming the four variables. Drivers (create/suspend/top-up), rides, and fare/commission screens. **Not yet usable in production: nobody can log in until Firebase is configured** | ~~ 4 files, no `index.html`, no `main.tsx`, no components, no routes. `vite build` fails with "Could not resolve entry module index.html". Operators cannot approve drivers, resolve disputes, top up wallets, or change fares |
| **P1** | **N-1** | **FIXED (code)** | Was: the worker logged `push.pending` and dropped every job, and there was no table a device token could be stored in. Now: migration 0007 `device_tokens`, `POST`/`DELETE /v1/devices` (added to the API contract, not invented), an `FcmSender` on FCM HTTP v1 using `jose` rather than pulling `firebase-admin` and gRPC into a 4-core box, and the worker actually delivers. 13 tests. **Still CONFIGURATION REQUIRED**: without `FCM_SERVICE_ACCOUNT_JSON` nothing is delivered - reported as failure, never success | ~~ A driver whose app is backgrounded receives no ride offers; offers expire in 15s, so drivers must keep the app open and foregrounded to earn |
...
| **P0** | **D-17** | **FIXED** | **`migrate:up` reported SUCCESS and applied nothing.** The CLI guard compared `import.meta.url` against a hand-built `file://` + `process.argv[1]` string. On Windows that compares a forward-slash URL against a backslash path, never matches, so the entry block never ran and the process exited 0 leaving an empty schema. A migration tool that silently no-ops is worse than one that fails - everyone believes the schema is migrated. Fixed with `pathToFileURL`. Found the first time the migrations were ever executed |
| P1 | D-18 | **CLOSED 2026-08-23** | The script no longer uses `--experimental-strip-types`; it is `pnpm build && node dist/db/migrate.js up`, which resolves its `./migrations.js` import from the compiled output. Verified end to end against real PostgreSQL 17.11: `MIGRATE_EXIT=0`, "No pending migrations". The register had been left saying `open` after the fix landed — corrected here |
| P2 | D-19 | **closed** | `pnpm seed` points at `src/db/seed.ts`, which does not exist. Seed data is referenced by CLAUDE.md 11 and by the setup docs; the script has never been runnable. Recorded rather than invented - writing seed data is a product decision about what a fresh environment should contain | CLOSED - `src/db/seed.ts` written; runs, is idempotent (4 users then 0), and refuses production.
...
| P2 | D-7 | **closed** | Production PgBouncer guard is a string match, not a URL parse | CLOSED - guard parses the URL; caught that `postgres://host/db` (no port, defaults to 5432) was missed entirely. 4 tests.
