...
## D-024 — بوابة Wayl: سكّة تحصيل حيّة واحدة، من جهة السائق فقط

**التاريخ:** 2026-09-06 · **القرار لك (المالك)، ونُفِّذ بناءً عليه.**

**الحالة:** مُنفَّذ خلف مفتاحَين مُطفأين. لا يمسّ أي دينار حتى تُشغّلهما.

### ما تغيّر في النطاق

`CLAUDE.md` §2 كانت تمنع أي بوابة دفع حيّة، و§7 تطلب `GatewayProvider` يرمي
`NotImplementedError`. عُدِّلت الاثنتان اليوم للسماح بـ**بوابة واحدة**، وبقيّة
المنع باقية كما هي.

**ما دخل:** اشتراكات السائقين، وشحن محفظة السائق.
**ما بقي خارجاً:** أجرة الرحلات. الرحلة نقداً، والراكب لا يُدخل بطاقة.

**السبب أن هذا الحدّ ليس تحفّظاً بل هندسة:** أجرة الرحلة تُحسم في نهاية رحلة،
على شبكة بغداد، والسائق واقف ينتظر. فشل الشبكة هناك يعني راكباً لا يستطيع
النزول ورحلة لا تُغلق. أمّا الاشتراك فيُدفع مرّة في الشهر، من مكان يختاره
السائق، وفشله لا يُعطّل شيئاً — يُعاد المحاولة. نفس الشيفرة، خطر مختلف تماماً.

### الدفتر: أين تُقيَّد عمولة المزوّد

هذا كان الجزء الصعب. §6.2 تُثبّت أربعة أنواع حسابات ولا يوجد بينها «عمولة
مزوّد»، و§6.3 تمنع التعديل — أي قيد خاطئ هنا يبقى للأبد.

25,000 وصلت، أخذ المزوّد 1,225:

```
معاملة 1:  DEBIT  MANUAL_ADJUSTMENT   25,000   مال دخل من خارج النظام
           CREDIT PLATFORM_REVENUE    25,000   الاشتراك تم كسبه
معاملة 2:  DEBIT  PLATFORM_REVENUE     1,225   حصّة المزوّد
           CREDIT MANUAL_ADJUSTMENT    1,225   خرجت إليه
```

**معاملتان لا واحدة.** قيدٌ واحد صافٍ بـ23,775 متوازنٌ حسابياً وصحيحٌ في
المجموع — ويُدمّر نهائياً جواب سؤال «كم كلّفتنا البوابة؟». نهائياً لأن الدفتر
لا يُعدَّل: لا يمكن العودة لتفصيله لاحقاً. الآن الإجمالي والعمولة كلاهما صفّ
قابل للاستعلام، و`PLATFORM_REVENUE` يصفو على 23,775 وحده.

**`DRIVER_WALLET` لا تُمسّ.** السائق دفع لمزوّد، لا من أرباحه. خصمُ المحفظة
هنا يُحاسبه مرّتين — نفس الفخّ الذي وثّقه D-020 للنقد.

**الاشتراك يُمنح في نفس المعاملة، بشحن صفر.** `SubscriptionService.grant` يكتب
زوجه النقدي الخاص للبيع النقدي؛ المال هنا مُقيَّد أعلاه أصلاً، والسماح له
بالكتابة مرّة ثانية يُسجّل 25,000 مرّتين.

### الاستطلاع هو المرجع، لا الـwebhook

الـwebhook تسريعٌ للتجربة، والمسح الدوري هو ما يقرّر. webhook لا يصل يترك مالاً
محصَّلاً واشتراكاً غير ممنوح؛ الاستطلاع **لا يمكن أن يفوته شيء لأنه يسأل**.

وتوقيع Wayl **لا يحمل طابعاً زمنياً**، فالطلب الملتقط يبقى صالحاً تشفيرياً
للأبد. التوقيع لازم وغير كافٍ: كل webhook مقبول يُنزع تكراره على معرّف الحدث في
`payment_webhook_events` قبل أن يُسمح له بكتابة أي شيء. وحالة لا نعرفها تُقرأ
`PENDING` أبداً `PAID` — «لا أعرف» تعني «ليس بعد»، فيبقى الصفّ للمسح التالي.

### مفتاحان، لا واحد

1. `WAYL_TOKEN` + `WAYL_WEBHOOK_SECRET` في البيئة — نشر.
2. `platform_config.gateway_enabled = 'true'` — ليس نشراً (§6.5).

تقدر تغلق السكّة أثناء عطل دون انتظار أحد، واستعادة نسخة احتياطية لا تفتحها
بصمت على بيانات اعتماد ناقصة. مع نقص أيٍّ منهما: 501، ولا يُعرض على سائق رابط
دفع لا يمكن دفعه.

### لماذا جدول جديد لا صفّ في `payments`

`payments.ride_id` هي `NOT NULL` و`UNIQUE`. اشتراكٌ ليس رحلة. حشره هناك يتطلّب
إسقاط قيدٍ يحمي كل دفعة رحلة في النظام، لأجل صفّ ليس رحلة أصلاً.

### ما يثبت هذا

`real-gateway-ledger.test.ts` — عشرة اختبارات على PostgreSQL حقيقي بمشغّلاته.
جُرِّبت بقلب اتجاه قيد العمولة: سقط اختباران. ولاحظ أنّ فحص التوازن **نجح** رغم
الخطأ — التوازن ليس صحّة، وهذا سبب وجود هذا الملف على قاعدة حقيقية لا على
`FakeDatabase`. مرّتان في هذا المستودع وافقت نسخةٌ مزيّفة على خطأ.

**Reverse it if:** لم تُوقَّع اتفاقية مع Wayl، أو رفضوا حساباً حيّاً. اترك
`gateway_enabled=false` ولا تضع البيانات — النظام يعمل كما كان بالضبط، والنقد
لم يُمسّ. حذفه بالكامل: `0016_gateway_payments.down.sql` يُسقط الجدول فعلاً
(لا قيمة enum مضافة، فلا شيء عالق).
